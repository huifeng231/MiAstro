...
from utils.err_msg import mysql_msg
from utils.exceptions import FuncArgsError, HttpError, MysqlDuplicateKeyError, MysqlError, QueryArgsError
from utils.utils import verify_message
...
class AIOMysqlClient(object):
    ...
    def init_app(self, app, *, username=None, passwd=None, host=None, port=None, dbname=None,
        ...
        async def open_connection(app_, loop):
            ...
            self.aio_engine = await create_engine(user=username, db=dbname, host=host, port=int(port),
                                                  password=passwd, maxsize=int(pool_size), charset="utf8")
